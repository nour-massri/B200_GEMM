#pragma once
// H100 (SM90 / Hopper) PTX helper wrappers.
// All WGMMA, TMA, and mbarrier primitives used across matmul kernels.
...
// ---------------------------------------------------------------------------
// WGMMA shared memory descriptor
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// WGMMA PTX wrappers (m64nNk16, BF16->F32)
// ---------------------------------------------------------------------------
...
// Dispatcher — selects the right wgmma variant based on WGMMA_N
...
__device__ __forceinline__ void wgmma(float d[WGMMA_N/16][8], bf16* sA, bf16* sB) {
    ...
    if constexpr (WGMMA_N == 256) wgmma256<ScaleD, ScaleA, ScaleB, TransA, TransB>(d, sA, sB);
    if constexpr (WGMMA_N == 192) wgmma192<ScaleD, ScaleA, ScaleB, TransA, TransB>(d, sA, sB);
    if constexpr (WGMMA_N == 128) wgmma128<ScaleD, ScaleA, ScaleB, TransA, TransB>(d, sA, sB);
    if constexpr (WGMMA_N == 64)  wgmma64 <ScaleD, ScaleA, ScaleB, TransA, TransB>(d, sA, sB);
    if constexpr (WGMMA_N == 32)  wgmma32 <ScaleD, ScaleA, ScaleB, TransA, TransB>(d, sA, sB);
}

// ---------------------------------------------------------------------------
// TMA tensor map creation
//
// 2D variant (M2-M6): tiles up to 64 columns wide, descriptor written to *tma_map
// 3D variant (M8-M12): supports wide tiles via 3D tiling, returns by value
// ---------------------------------------------------------------------------
...
__host__ inline void create_tensor_map(CUtensorMap *tma_map, bf16* gmem_ptr,
    ...
    uint64_t gmem_prob_stride[5] = {sizeof(bf16),
...
__host__ static inline CUtensorMap create_tensor_map(bf16* gmem_ptr,
    ...
    uint32_t smem_box_shape[5]   = {padding ? 72u : 64u,
                                    uint32_t(BlockMajorSize), uint32_t(BlockMinorSize/64), 1, 1};
    uint32_t smem_box_stride[5]  = {1, 1, 1, 1, 1};
...
// 3D TMA async load (works for tiles wider than 64 columns via 3D indexing)
...
__device__ static inline void load_async_multicast(bf16 *dst, void const* src_tma_map,
                                                   uint64_t* bar,
                                                   int global_col_idx, int global_row_idx,
